...
window.onload = function() {
    ...
    var ys = new Vue({
        ...
        methods: {
            ...
            showmsgall: function() {
                axios.get('/api/client/umsgshowall', {
                    params: {
                        token: localStorage.getItem("token")
                    }
                }).then(function(resp) {
                    // console.log(resp.email)
                    ys.Tagname = resp.data.tagname
                    ys.Name = resp.data.name
                    ys.Sexy = resp.data.sexy
                    ys.Idcard = resp.data.idcard
                    ys.Email = resp.data.email
            ...
            smsg: function() {
                ...
                params.append('sex', this.Sexy)
                params.append('idcard', this.Idcard)
                params.append('email', this.Email)
                params.append('token', localStorage.getItem("token"))
                axios.post('/api/client/msg', params, {
                    headers: {
            ...
            send: function() {
                ...
                params.append('youbian', this.youbian);
                params.append('names', this.names);
                params.append('tel', this.tel);
                params.append('token', localStorage.getItem("token"))
                axios.post('/api/client/send', params, {
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded'
                    }
                }).then(function(resp) {
            ...
            updateCity: function() {
                ...
                for (var i in that.arr) {
                    var obj = that.arr[i];
                    if (obj.name == that.prov) {
                        that.cityArr = obj.sub;
                        break;
                    }
                }
                that.city = that.cityArr[1].name;
            },
            updateDistrict: function() {
                var that = this
                for (var i in that.cityArr) {
                    var obj = that.cityArr[i];
                    if (obj.name == that.city) {
                        that.districtArr = obj.sub;
                        break;
                    }
                }
                if (that.districtArr && that.districtArr.length > 0 && that.districtArr[1].name) {
                    that.district = that.districtArr[1].name;
                } else {
                    that.district = '';
                }
            },
    ...
    var atn = document.getElementsByClassName("atn")[0];
    var li = document.getElementsByClassName("li");
    var img = document.getElementsByClassName("right");
    var coun = 1;
    var num = 11;
    var bottom = document.getElementsByClassName("status");
    var con = document.getElementsByClassName("con");
    var box2 = document.getElementsByClassName("box2");
    var localinfo = document.getElementsByClassName("localInfo");
    var str = ["请选择省/市/区/街道", "道路、小区、门牌号", "请填写邮政编码", "长度不超过25个字符", "长度不超过11字符"];
    var path = atn.src;
    ...
    atn.onclick = function() {
            ...
            if (atn.src == path) {
                // console.log("b");
                atn.src = "static/img/img/redheart.png";
                atn.title = "取消关注";
            } else {
                atn.src = "static/img/img/grayheart.png";
                atn.title = "关注";
            }
        }
        //默认选择我的账户

    img[0].src = "static/img/img/clickRight.png"
    li[0].style.backgroundColor = "white";
    li[0].style.borderBottom = "1px solid lightgray";
    li[0].style.borderTop = "1px solid lightgray";
    //导航栏点击事件
    for (var i = 0; i < num; i++) {
        (function(n) {
            li[n].onclick = function() {
                for (var j = 0; j < num; j++) {
                    img[j].src = "static/img/img/right.png";
                    li[j].style.backgroundColor = "rgb(241, 241, 239)";
                ...
                if (n == 2) {
                    if (coun % 2 == 1) {
                        ...
                        img[n].src = "static/img/img/clickDown.png"
                        li[3].style.display = "block";
                        li[4].style.display = "block";
                        for (var k = 0; k < box2.length; k++) {
                            box2[k].style.height = "750px";
                        }
                    } else {
                        img[n].src = "static/img/img/right.png";
                        li[3].style.display = "none";
                        li[4].style.display = "none";
                        for (var k = 0; k < box2.length; k++) {
                            box2[k].style.height = "670px";
                        }
                    }
                    coun++;
                } else {
                    img[n].src = "static/img/img/clickRight.png";
                }
                li[n].style.backgroundColor = "white";
                li[n].style.borderBottom = "1px solid lightgray";
                li[n].style.borderTop = "1px solid lightgray";
    ...
    for (var i = 0; i < bottom.length - 1; i++) {
        bottom[i].style.borderBottom = "none";
    }

    //聚焦事件
    for (var i = 0; i < localinfo.length; i++) {
        localinfo[i].style.color = "#999";
        (function(n) {
            localinfo[n].onfocus = function() {
                if (localinfo[n].value == str[n]) {
                    console.log("a")
                    localinfo[n].value = "";
                    localinfo[n].style.color = "black";
                }
            }
            localinfo[n].onblur = function() {
                if (localinfo[n].value == "") {
                    localinfo[n].value = str[n];
                    localinfo[n].style.color = "#999";
                }
            }
        }(i))
    }
